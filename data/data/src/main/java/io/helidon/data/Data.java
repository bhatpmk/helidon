...
public final class Data {
    ...
    public @interface Query {
        ...
        /**
         * Query that returns the total number of rows matching {@link #value()} before pagination is applied.
         * <p>
         * The JDBC provider requires this member when a repository method returns {@link Page}. It is not used for
         * {@link Slice}, because a slice does not expose the total result size. The query must return exactly one
         * non-null, non-negative integral value. Providers that can derive a count query may ignore this member.
         *
         * @return count query, or an empty string when no count query is required
         */
        String count() default "";
    }

    /**
     * Request keys generated by the database for an ordinary SQL operation declared with {@link Data.Query}.
     * <p>
     * The repository method return type defines generated-key cardinality and mapping. A scalar or projection return
     * type requires exactly one generated-key row, {@link Optional} accepts zero or one row, and a collection returns
     * all generated-key rows. Without this annotation, numeric results from insert and update operations represent
     * affected row counts rather than generated keys.
     * <p>
     * SQL clauses such as {@code RETURNING} or {@code OUTPUT} produce ordinary row results and do not require this
     * annotation. Availability, shape, and ordering of JDBC generated keys are database and driver specific.
     */
    @Target(ElementType.METHOD)
    @Retention(RetentionPolicy.SOURCE)
    public @interface GeneratedKeys {
        /**
         * Generated-key column names requested from the JDBC driver.
         * <p>
         * An empty array requests the driver-defined generated-key columns. Drivers may not support selecting generated
         * keys by column name.
         *
    ...
    /**
     * Explicit input metadata for a {@link Data.Call} method parameter.
     * <p>
     * Plain method parameters are inferred as callable inputs in call-marker order. This annotation is needed only when
     * the input requires an explicit JDBC index, a bind-marker name that differs from the Java parameter name, or JDBC
     * type information. It cannot be combined with {@link Data.InOut} on the same parameter.
     */
    @Target(ElementType.PARAMETER)
    @Retention(RetentionPolicy.SOURCE)
    public @interface In {
        /**
         * Sentinel used when no JDBC SQL type is declared.
         */
        int UNSPECIFIED_TYPE = Integer.MIN_VALUE;

        /**
         * One-based JDBC parameter index, or {@code -1} to infer the index from call-marker order.
         *
        ...
        /**
         * SQL bind-marker name when it differs from the Java method parameter name.
         * <p>
         * This member applies to named SQL bind markers. It is not a request to use the optional JDBC driver API for
         * binding a callable parameter by database parameter name.
         *
         * @return bind-marker name
         */
        String name() default "";

        /**
         * SQL type from {@code java.sql.Types}, or {@link #UNSPECIFIED_TYPE} to let the driver infer it.
         *
         * @return SQL type
         */
        int type() default UNSPECIFIED_TYPE;

        /**
         * Scale or length supplied to JDBC typed object binding, or {@code -1} when unspecified.
         *
         * @return scale or length
         */
        int scale() default -1;

        /**
         * Database type name used by JDBC when binding a typed SQL {@code NULL}.
         *
