...
/**
 * Emits JDBC row reducer code for repository methods that return relationship graphs.
 * <p>
 * A reducer is needed when a flat result set contains collection paths such as
 * {@code phones.id} or {@code phones.tags.name}. This generator converts mapping
 * metadata into a small tree model, validates that each node has a key, and emits
 * accumulator classes that de-duplicate parent and child rows without reflection.
 */
final class JdbcReducerCodegen {
    ...
    boolean reducerRequired(TypeName resultType, TypedElementInfo methodInfo, String statement) {
        if (explicitReducer(resultType, methodInfo).isPresent()) {
            return true;
        }
        // Automatic reducers are generated only when mapping metadata crosses collection properties.
    ...
    void appendReducer(Method.Builder builder, TypeName resultType, TypedElementInfo methodInfo, String statement) {
        Optional<TypeName> explicitReducer = explicitReducer(resultType, methodInfo);
        if (explicitReducer.isPresent()) {
            // User-provided reducers are instantiated directly by generated repository code.
            builder.addContent("new ")
                    .addContent(explicitReducer.get())
                    .addContent("()");
            return;
        }
    ...
    private Optional<TypeName> explicitReducer(TypeName resultType, TypedElementInfo methodInfo) {
        ...
        TypeName expectedType = TypeName.builder(JDBC_ROW_REDUCER)
                ...
                .build();
        reducerInfo.findInHierarchy(expectedType)
                .orElseThrow(() -> new CodegenException("@Data.ReduceWith explicit reducer must implement "
                                                                + expectedType.fqName()
                                                                + ": "
                                                                + reducerType.fqName(),
                                                        methodInfo.originatingElement()));
        validateExplicitReducer(reducerInfo, methodInfo);
        return Optional.of(reducerInfo.typeName());
    }

    private void validateExplicitReducer(TypeInfo reducerInfo, TypedElementInfo methodInfo) {
        if (reducerInfo.accessModifier() == AccessModifier.PRIVATE) {
            throw new CodegenException("@Data.ReduceWith explicit reducer must be accessible: "
                                               + reducerInfo.typeName().fqName(),
                                       methodInfo.originatingElement());
        }
        boolean defaultConstructor = reducerInfo.elementInfo()
                .stream()
                .filter(it -> it.kind() == ElementKind.CONSTRUCTOR)
                .findFirst()
                .isEmpty();
        boolean accessibleNoArgConstructor = reducerInfo.elementInfo()
                .stream()
                .filter(it -> it.kind() == ElementKind.CONSTRUCTOR)
        ...
        if (!defaultConstructor && !accessibleNoArgConstructor) {
            throw new CodegenException("@Data.ReduceWith explicit reducer requires an accessible "
    ...
    private Optional<ReducerModel> reducerModel(TypeName resultType, TypedElementInfo methodInfo, String statement) {
        ...
        // After the shape is known, validate labels, keys, and write access before emitting code.
        validateSourceLabels(statement, metadata, methodInfo);
    ...
    private MappingMetadata mappingMetadata(TypeName resultType, TypedElementInfo methodInfo, String statement) {
        ...
        // Dot-separated result labels can opt into automatic reducer mappings without annotations.
    ...
    private void validateSourceLabels(String statement, MappingMetadata metadata, TypedElementInfo methodInfo) {
        ...
        metadata.mappings()
                ...
                .stream()
                .map(Mapping::source)
                .filter(source -> !labels.contains(source))
                .findFirst()
                .ifPresent(source -> {
                    throw new CodegenException("JDBC reducer source label \"" + source
        ...
        metadata.keys()
                ...
                .findFirst()
                .ifPresent(source -> {
                    throw new CodegenException("JDBC reducer key label \"" + source
                                                       + "\" is not produced by SQL query. Available labels: "
                                                       + labels.labels(),
                                               methodInfo.originatingElement());
                });
    ...
    private boolean addMapping(NodeSpec root, Mapping mapping, TypedElementInfo methodInfo) {
        ...
        for (int i = 0; i < parts.length; i++) {
            ...
            // Each collection segment becomes a child node with its own key and accumulator map.
    ...
    private void applyKeys(NodeSpec root, Map<String, List<String>> keys, TypedElementInfo methodInfo) {
        visit(root, node -> {
            ...
            // Convention: an "id" scalar on each node is used as the reducer key when present.
    ...
    private void validateConstruction(NodeSpec root, TypedElementInfo methodInfo) {
        visit(root, node -> {
            if (node.typeInfo().kind() == ElementKind.CLASS) {
                ...
                node.children()
                        .values()
                        .forEach(child -> collectionWriter(node.typeInfo(), child.property(), methodInfo));
    ...
    private TypeName generateReducer(ReducerModel model, TypedElementInfo methodInfo) {
        ...
        // Generated reducers keep insertion order stable by using LinkedHashMap keyed by declared key values.
    ...
    private void generateAccumulator(InnerClass.Builder inner, NodeSpec node, TypedElementInfo methodInfo) {
        // One accumulator class is emitted per graph node; child accumulators de-duplicate nested rows.
        ...
        node.children()
                .values()
                .forEach(child -> inner.addMethod(method -> generateFinishCollectionMethod(method, child)));
    ...
    private void appendKey(ContentBuilder<?> builder, NodeSpec node) {
        // Composite keys are represented as ordered object lists for map lookup.
    ...
    private PropertyWriter writer(TypeInfo owner, PropertySpec property, TypedElementInfo methodInfo) {
        // Scalar properties require a direct writable member because reducer code uses no reflection.
    ...
    private PropertyWriter collectionWriter(TypeInfo owner, PropertySpec property, TypedElementInfo methodInfo) {
        // Collection properties can be populated through a field, getter-returned collection, or setter.
    ...
    private void addMapping(Map<String, Mapping> mappings, Map<String, Mapping> localTargets, Annotation annotation) {
        ...
        Mapping mapping = new Mapping(target, resolvedSource);
        if (localTargets.put(target, mapping) != null) {
            throw new CodegenException("@Data.Map declares multiple mappings for target: " + target);
        }
        mappings.put(target, mapping);
    }
    ...
    private List<String> resultLabels(String statement) {
        return List.copyOf(JdbcResultLabels.parse(statement).labels());
    }
    ...
    /**
     * Mutable build-time description of one object in the reduced result graph.
     */
