 ...
 *
 * JdbcStatementPlan uses update(...) for INSERT, UPDATE, and DELETE because JDBC itself treats all three as update operations.
 *
 * In JDBC, these statements normally execute through:
 *   PreparedStatement.executeUpdate()
 *   PreparedStatement.executeLargeUpdate()
 *   and return an affected-row count.
 *
 * The current kernel mirrors that model:
 *   JdbcStatementKind.UPDATE
 *   JdbcOperations.update(...)
 *   JdbcKernel.update(...)
 *   statement.executeLargeUpdate()
 *
 * So a SQL delete like DELETE FROM sometable WHERE id = ? is represented as:
 *   JdbcStatementPlan.update("DELETE FROM sometable WHERE id = ?")
 *
 *
