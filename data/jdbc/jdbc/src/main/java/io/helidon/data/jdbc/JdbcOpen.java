...
/**
 * A {@link JdbcAutoCloseable} with close handlers that throws {@link SQLException}s.
 *
 * @see #onClose(JdbcRunnable)
 */
@io.helidon.common.Api.Internal
public interface JdbcOpen extends JdbcAutoCloseable {

    /**
     * Returns an equivalent {@link JdbcOpen} with an additional close handler.
     *
     * @param closeHandler a non-{@code null} {@link Runnable} representing a task to run when this {@link JdbcOpen}
     * implementation is {@linkplain #close() closed}
     * @return an equivalent {@link JdbcOpen} with an additional close handler
