...
Use `@Data.Param` on repository method parameters when the binding contract
should be independent of Java parameter names. `@Data.Param("name")` binds an
argument to a named marker such as `:name`, and `@Data.Param(index = 1)` binds
an argument to the first positional marker. When any SQL-bound argument on a
method uses `@Data.Param`, all SQL-bound arguments on that method must use it.
The JDBC provider rejects duplicate explicit bindings and mixed named/positional
SQL markers.
